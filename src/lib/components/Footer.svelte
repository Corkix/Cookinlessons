<script>
	import loga from '$lib/images/hagalogo1.png';
</script>

<footer class="bg-gray-100">
	<div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
		<div class="flex justify-center">
			<img alt="Hagablänkets loga" src={loga} />
		</div>

		<article class="mx-auto mt-6 max-w-md text-center leading-relaxed">
			<h3>Hagablänket</h3>
			<p>070-603 95 82</p>

			<p>kontakt@hagablanket.se</p>

			<p>Stenberga Sänsleviken 1</p>
			<p>574 97, Vetlanda</p>
		</article>

		<ul class="mt-12 flex flex-wrap justify-center gap-6 md:gap-8 lg:gap-12">
			<li>
				<a class="text-gray-700 transition hover:text-gray-700/75" href="/"> Hem </a>
			</li>

			<li>
				<a class="text-gray-700 transition hover:text-gray-700/75" href="/njutbart"> Njutbart </a>
			</li>

			<li>
				<a class="text-gray-700 transition hover:text-gray-700/75" href="/omOss"> Om oss </a>
			</li>

			<li>
				<a class="text-gray-700 transition hover:text-gray-700/75" href="/kontakt"> Kontakt </a>
			</li>

			<li>
				<a class="text-gray-700 transition hover:text-gray-700/75" href="/">
					Stenberga Missionsförsamling
				</a>
			</li>
		</ul>

		<ul class="mt-12 flex justify-center gap-6 md:gap-8">
			<li>
				<a
					href="/"
					rel="noreferrer"
					target="_blank"
					class="text-gray-700 transition hover:text-gray-700/75"
				>
					<span class="sr-only">Facebook</span>
					<svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
						<path
							fill-rule="evenodd"
							d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
							clip-rule="evenodd"
						/>
					</svg>
				</a>
			</li>
		</ul>
	</div>
	<div class="flex justify-center p-1">
		<p class="text-sm">Copyright © 2023 Hagablänket Alla rättigheter förbehållna</p>
	</div>
</footer>
